...
- Make RPC server failures survivable and diagnosable: build the server with `panic = "unwind"` so a panic in one request thread no longer aborts the whole process (and the mutex poison-recovery path actually runs), log panics to the server log, and—when the server vanishes while the container stays up—probe the container cgroup (`memory.events`, `memory.peak`) to report whether it was a cgroup OOM kill (including whole-cgroup `memory.oom.group` kills, which `oom_score_adj` cannot prevent) or a server-side crash, instead of guessing "likely process-level OOM or crash".
