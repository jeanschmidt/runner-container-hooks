...
interface ContainerDeathEvidence {
  oomKill: number
  oomGroupKill: number
  memPeak: number | null
...
function parseContainerDeathProbe(out: string): ContainerDeathEvidence {
...
async function diagnosePodFailure(
  ...
  if (phase === 'Running' && containerStatus?.state?.running) {
    ...
    const ev = parseContainerDeathProbe(probe)
    const memCtx =
